...
# Definition
----
Ein Array (Feld) ermöglicht das speichern und verwalten von Daten des gleichen Typs innerhalb eines Objekts. Jedes Element im Array enthält eine Indexnummer.

---

# Array erzeugen
```java
// new Komponententyp[Feldgröße];
// Beispiel: 
new int[4];
```
- Die Daten sind alle von Typ int.
- Die Größe des Arrays wird beim Erzeugen festgelegt und lässt sich im Nachhinein nicht ändern.

## Initalisierungsliste
```java
// new Kompontentyp[]{a,b,c}
// Beispiel:
new int[]{0, 2, 4, 6, 8};
```
- Mit der Initalisierungsliste können Werte im Array hinzugefügt werden.



# Zugriff im Array (Index)
## Indexgrenzen
```java
// Die Indexgröße kann mit .length zugegriffen werden. Nicht zu verwechseln mit (String).length()
int [] Folge;
Folge.length();
```
...
### Kopieren von Arrays
```java
int[] kopie = new int[variabel.length];
```

### Wachsende Felder.
```java
// Arrays bekommen eine feste Größe beim Erzeugen. Soll dieses Feld trotzdem wachsen, sollte ein weiteres Feld erzeugt werden und das alte wird verworfen.
...
## Vorlesung (Sonstiges)
```java
// Möglichkeiten zum Erzeugen eines leeren Arrays
new int [0];
new int[]{};
```

``` java
public static void f(int[] feld) {
...
int[] m = new int[10];
f(m);
// Hat m jetzt die Länge 11? Nein. Die Methode kann nicht von Typ void sein.
// Richtige Version:
public static int[] f(int[] feld) {
