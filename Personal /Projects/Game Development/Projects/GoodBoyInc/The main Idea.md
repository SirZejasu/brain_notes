#Practice 
#GameDevelopment
#Idea 

---
# Planing
- Map
- Gameplay / Minigames



# Handlung
- Unmoralische Arbeitspraktiken

# Definition von Unethisch
**Bedeutungen:**  
1. nicht der Ethik und Moral entsprechend

**Synonyme:**  
1. unmoralisch, verwerflich

# Referenz aus echten Leben
- Sexuelle Belästigung bei Blizzard
- Unmorialische Arbeitsumstände bei Apple und Amazon

# Game Mechanics
- Der Start ähnelt den von Lethal Company, Repo und Content Warning
	  - Spieler muss jede Runde / Monat eine bestimmte Menge an Geld verdienen.
	  - Verdient extra Geld, indem unethische Arbeitspraktiken ausgeführt werden
		  - Cryptomining.
		  - Dokumente mit kritischen Informationen werden ausgedruckt und verkauft.
		  - ...

# Story / Progression
1. Arbeitnehmer sieht immer weiter unethische Praktiken im Unternehmen.
2. Wird inspiriert, unethische Optionen werden freigeschaltet.
3. Arbeitstag läuft.
4. Nächsten Morgen kommt ein Skandal, wenn Dokumente veröffentlicht werden.

## Typischer Ablauf
1. Tutorial
	1. Regulärer Arbeitsablauf
	2. Erste unethische Arbeitsaufgab
	3. Wird ein Dokument verkauft / veröffentlicht?
		1. Ja, Skandal wird veröffentlicht. Spieler muss am nächsten Tag vorsichtiger sein aber bekommt mehr Optionen.
		2. Nein. Weniger Optionen aber ..?
2. 2.Tag beginnt.
	1. Tag wiederholt sich, mit mehr Kontext zur "Geschichte"
	2. Jeder Tag gibt die Möglichkeit Optionen freizuschalten und in der Geschichte voranzuschreiten.
		